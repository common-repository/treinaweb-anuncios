...
add_action('add_meta_boxes', 'add_metabox_tipo_e_link');

function add_metabox_tipo_e_link() {
...
add_action('save_post', 'savar_metas_anuncio');


function savar_metas_anuncio() {
    ...
    update_post_meta($post->ID, 'tipo_anuncio', sanitize_text_field($tipo_anuncio));
    update_post_meta($post->ID, 'link_anuncio', sanitize_text_field($link_anuncio));
}
